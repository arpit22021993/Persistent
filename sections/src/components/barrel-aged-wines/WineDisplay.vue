  ...
  .wine-display
    .page-content-wrap
      .flex-container(ref="container")
        span.col(v-for="(wine, index) in barrelAged" :style="{paddingBottom: getWinePadding(index)}")
...
import WineItem from '../barrel-aged-wines/WineItem'
import WineItemDropDown from '../barrel-aged-wines/WineItemDropDown'
...
export default {
  ...
  computed: {
    ...mapGetters({
      barrelAged: 'barrelAged/wineList',
    }),
    ...mapState({
      activeWineIndex: state => state.barrelAged.activeWine,
  ...
  methods: {
    ...
    updateActiveWine () {
      this.activeWine = this.barrelAged[this.activeWineIndex]
    },
  },
  components: {
    WineItem,
