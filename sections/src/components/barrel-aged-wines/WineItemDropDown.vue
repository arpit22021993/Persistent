  ...
  .wine-item-dropdown.bg(v-show="showDropDown", ref="display")
    ...
    .wine-item-content
      ...
      .icons
        router-link.link.mr(
          to="/where-to-buy",
          :id="'barrelaged_wtb_' + activeWine.index",
          v-on:keydown.prevent.tab.native="focusEAD",
          v-on:keydown.prevent.shift.tab.native="focusWineBottle"
        ) Where to Buy
        //- <router-link to="/eat-and-drink" class="link mr" :id="'barrelaged_ead_'+activeWine.index">Eat & Drink</router-link>
        a.drizlyIcon(
          href="https://drizly.com/wine-brands/woodbridge-by-robert-mondavi/b1119",
          :id="'barrelaged_drizzly_' + activeWine.index",
          target="_blank",
          @keydown.prevent.tab.exact="gotoNextBottleEAD()",
          @keydown.prevent.shift.tab="focusEAD()"
        )
          drizly
        a.instaCartIcon(
          :href="activeWine.instacartLink",
          target="_blank",
          :id="'barrelaged_insta_' + activeWine.index",
          @keydown.prevent.tab.exact="gotoNextBottleInsta()",
          @keydown.prevent.shift.tab="focusDrizzly()"
        )
          instacart(v-show="activeWine.instacartLink")
</template>
...
export default {
  ...
  methods: {
    ...
    getBottleImgUrl(name) {
      ...
      return require(`../../assets/barrel-aged-wines/bottles/${name}.png`);
    },
    ...
    getImgUrl(prefix, name, size) {
      if (size === "small") {
        return require(`../../assets/barrel-aged-wines/${prefix}/${name}-small.png`);
      } else {
        return require(`../../assets/barrel-aged-wines/${prefix}/${name}.png`);
      }
    },
    // },
    gotoNextBottleEAD: function () {
      if (
        document.getElementById(
          "barrelaged_insta_" + this.activeWine["index"]
        ) == ""
      ) {
        if (this.activeWine["index"] != this.wineList.length - 1) {
          var nextFocus = this.activeWine["index"] + 1;
          document.getElementById("barrelagedbottle_" + nextFocus).focus();
        } else {
          document.getElementById("our-wines").focus();
        }
      } else {
        document
          .getElementById("barrelaged_insta_" + this.activeWine["index"])
          .focus();
      }
    },
    gotoNextBottleInsta: function () {
      if (this.activeWine["index"] != this.wineList.length - 1) {
        var nextFocus = this.activeWine["index"] + 1;
        document.getElementById("barrelagedbottle_" + nextFocus).focus();
      } else {
    ...
    focusDrizzly: function () {
      ...
      document.getElementById("barrelaged_drizzly_" + nextFocus).focus();
    },
    ...
    focusEAD: function () {
      var nextFocus = this.activeWine["index"];
      document.getElementById("barrelaged_ead_" + nextFocus).focus();
    },
  },
  components: {
    Drizly,
