  ...
  .wine-display
    .page-content-wrap
      .flex-container(ref="container")
        span.col(v-for="(wine, index) in grabAndGo" :style="{paddingBottom: getWinePadding(index)}")
...
import WineItem from '../grab-and-go/WineItem'
import WineItemDropDown from '../grab-and-go/WineItemDropDown'
...
export default {
  ...
  computed: {
    ...mapGetters({
      grabAndGo: 'grabAndGo/wineList',
    }),
    ...mapState({
      activeWineIndex: state => state.grabAndGo.activeWine,
  ...
  methods: {
    ...
    updateActiveWine () {
      this.activeWine = this.grabAndGo[this.activeWineIndex]
    },
  },
  components: {
    WineItem,
