  ...
  .wine-item-dropdown.bg(v-show="showDropDown", ref="display")
    ...
    .wine-item-content
      ...
      .icons
        router-link.link.mr(
          to="/where-to-buy",
          :id="'grabandgo_wtb_' + activeWine.index",
          v-on:keydown.prevent.tab.native="focusEAD",
          v-on:keydown.prevent.shift.tab.native="focusWineBottle"
        ) Where to Buy
        //- router-link to="/eat-and-drink" class="link mr" :id="'grabandgo_ead_'+activeWine.index" Eat & Drink
        a.instaCartIcon(
          :href="activeWine.instacartLink",
          target="_blank",
          :id="'grabandgo_insta_' + activeWine.index",
        ...
        a.drizlyIcon(
          ...
          :id="'grabandgo_drizzly_' + activeWine.index",
          target="_blank",
          @keydown.prevent.tab.exact="gotoNextBottleDrizzly()",
          @keydown.prevent.shift.tab="focusInsta()"
        )
          drizly(v-show="activeWine.drizlyLink")
</template>
...
export default {
  ...
  methods: {
    ...
    getBottleImgUrl(name) {
      ...
      return require(`../../assets/grab-and-go/bottles/${name}.png`);
    },
    ...
    gotoNextBottleInsta: function () {
      if (
        typeof document.getElementById(
          "grabandgo_drizzly_" + this.activeWine["index"]
        ) != "undefined" &&
        ...
        if (this.activeWine["index"] != this.wineList.length - 1) {
          ...
          document.getElementById("grabandgobottle_" + nextFocus).focus();
        } else {
          document.getElementById("our-wines").focus();
        }
      } else {
        document
          .getElementById("grabandgo_drizzly_" + this.activeWine["index"])
          .focus();
      }
    },
    gotoNextBottleDrizzly: function () {
      if (this.activeWine["index"] != this.wineList.length - 1) {
        var nextFocus = this.activeWine["index"] + 1;
        document.getElementById("grabandgobottle_" + nextFocus).focus();
      } else {
    ...
    focusInsta: function () {
      ...
      document.getElementById("grabandgo_insta_" + nextFocus).focus();
    },
    ...
    focusEAD: function () {
      var nextFocus = this.activeWine["index"];
      document.getElementById("grabandgo_ead_" + nextFocus).focus();
    },
  },
  components: {
    Instacart,
